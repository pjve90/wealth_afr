# Model with current absolute levels, wealth change, and moving variance ----

# The code in this script is meant to fit a Bayesian model that aims to predict the probability of first birth by  the interaction between absolute wealth and long-term wealth variability.

#Load packages
#install.packages("cmdstanr")
library(cmdstanr)
#install.packages("rethinking")
library(rethinking)
#install.packages("scales")
library(scales)
#install.packages("corrplot")
library(corrplot)

## Data wrangling of real data ----

#Load data
real_data <- read.csv("Data/dataf.csv")[,-1]
head(real_data)

# Age at first reproduction ----

#create a matrix to store the age-specific age of censor
afr_matrix <- matrix(nrow=nrow(real_data),ncol=max(real_data$aoc)+1)
#calculate for each age when the woman is censored (1) or not (0)
for(i in 1:nrow(afr_matrix)){
  afr <- real_data$afr[i] + 1 #adding 1 so if she reproduces in the same year as registered = 1
  aoc <- real_data$aoc[i] + 1 #adding 1 so if she is censored in the same year as registered = 1
  if(!is.na(afr)){
    afr_matrix[i,1:(afr-1)] <- 0
    afr_matrix[i,afr] <- 1
  } else{
    afr_matrix[i,1:aoc] <- rep(0,length(afr_matrix[i,1:aoc]))
  }
}
#check the data
afr_matrix
#check the age-specific probability of FR
apply(afr_matrix,2,sum,na.rm=T)/apply(afr_matrix,2,function(x)sum(!is.na(x)))
#plot the Cumulative probability of first birth
plot(cumprod(1-apply(afr_matrix,2,sum,na.rm=T)/apply(afr_matrix,2,function(x)sum(!is.na(x))))~c(1:(max(real_data$aoc)+1)),
     xlab="Age",
     ylab="Cumulative probability of first birth",
     ylim=c(0,1))

#Current absolute wealth ----

#Current absolute wealth
#create matrix to store the amount of wealth at each age
absw_matrix <- matrix(nrow = nrow(real_data),ncol=max(real_data$aoc)+1)
#calculate for each age the amount of wealth the household of a woman has, based on each census
#95
for(i in 1:nrow(absw_matrix)){
  absw <- real_data$absw95[i]
  age_absw <- real_data$age_absw95[i] + 1 #adding 1 so if she reproduces/censors in the same is registered = 1
  if(!is.na(age_absw) & age_absw <= (real_data$aoc[i]+1)){
    absw_matrix[i,age_absw] <- absw
  } else
    if(!is.na(age_absw) & age_absw > (real_data$aoc[i]+1)){
      absw_matrix[i,(real_data$aoc[i]+1)] <- NA
    } else{
      absw_matrix[i,age_absw] <- NA
    }
}
#check data
absw_matrix
#98
for(i in 1:nrow(absw_matrix)){
  absw <- real_data$absw98[i]
  age_absw <- real_data$age_absw98[i] + 1 #adding 1 so if she reproduces/censors in the same is registered = 1
  if(!is.na(age_absw) & age_absw <= real_data$aoc[i]){
    absw_matrix[i,age_absw] <- absw
  } else
    if(!is.na(age_absw) & age_absw > (real_data$aoc[i]+1)){
      absw_matrix[i,(real_data$aoc[i]+1)] <- NA
    } else{
      absw_matrix[i,age_absw] <- NA
    }
}
#check data
absw_matrix
#00
for(i in 1:nrow(absw_matrix)){
  absw <- real_data$absw00[i]
  age_absw <- real_data$age_absw00[i] + 1 #adding 1 so if she reproduces/censors in the same is registered = 1
  if(!is.na(age_absw) & age_absw <= real_data$aoc[i]){
    absw_matrix[i,age_absw] <- absw
  } else
    if(!is.na(age_absw) & age_absw > (real_data$aoc[i]+1)){
      absw_matrix[i,(real_data$aoc[i]+1)] <- NA
    } else{
      absw_matrix[i,age_absw] <- NA
    }
}
#check data
absw_matrix
#02
for(i in 1:nrow(absw_matrix)){
  absw <- real_data$absw02[i]
  age_absw <- real_data$age_absw02[i] + 1 #adding 1 so if she reproduces/censors in the same is registered = 1
  if(!is.na(age_absw) & age_absw <= real_data$aoc[i]){
    absw_matrix[i,age_absw] <- absw
  } else
    if(!is.na(age_absw) & age_absw > (real_data$aoc[i]+1)){
      absw_matrix[i,(real_data$aoc[i]+1)] <- NA
    } else{
      absw_matrix[i,age_absw] <- NA
    }
}
#check data
absw_matrix
#04
for(i in 1:nrow(absw_matrix)){
  absw <- real_data$absw04[i]
  age_absw <- real_data$age_absw04[i] + 1 #adding 1 so if she reproduces/censors in the same is registered = 1
  if(!is.na(age_absw) & age_absw <= real_data$aoc[i]){
    absw_matrix[i,age_absw] <- absw
  } else
    if(!is.na(age_absw) & age_absw > (real_data$aoc[i]+1)){
      absw_matrix[i,(real_data$aoc[i]+1)] <- NA
    } else{
      absw_matrix[i,age_absw] <- NA
    }
}
#check data
absw_matrix
#06
for(i in 1:nrow(absw_matrix)){
  absw <- real_data$absw06[i]
  age_absw <- real_data$age_absw06[i] + 1 #adding 1 so if she reproduces/censors in the same is registered = 1
  if(!is.na(age_absw) & age_absw <= real_data$aoc[i]){
    absw_matrix[i,age_absw] <- absw
  } else
    if(!is.na(age_absw) & age_absw > (real_data$aoc[i]+1)){
      absw_matrix[i,(real_data$aoc[i]+1)] <- NA
    } else{
      absw_matrix[i,age_absw] <- NA
    }
}
#check data
absw_matrix
#10
for(i in 1:nrow(absw_matrix)){
  absw <- real_data$absw10[i]
  age_absw <- real_data$age_absw10[i] + 1 #adding 1 so if she reproduces/censors in the same is registered = 1
  if(!is.na(age_absw) & age_absw <= (real_data$aoc[i]+1)){
    absw_matrix[i,age_absw] <- absw
  } else
    if(!is.na(age_absw) & age_absw > (real_data$aoc[i]+1)){
      absw_matrix[i,(real_data$aoc[i]+1)] <- NA
    } else{
      absw_matrix[i,age_absw] <- NA
    }
}
#check data
absw_matrix
#check the average of current absolute wealth at each age
apply(absw_matrix,2,mean,na.rm=T)
#plot it
plot(apply(absw_matrix,2,mean,na.rm=T)~c(1:ncol(absw_matrix)),
     xlab="Age",
     ylab="Average current absolute wealth",
     type="b"
)

#standardise the log-transformed current absolute wealth
std_absw_matrix <- matrix(standardize(log(as.vector(absw_matrix))),ncol=ncol(absw_matrix),nrow=nrow(absw_matrix))
#check the data
std_absw_matrix
#check the age-specific average of standardised current absolute wealth
apply(std_absw_matrix,2,mean,na.rm=T)
#plot it
plot(apply(std_absw_matrix,2,mean,na.rm=T)~c(1:(max(real_data$aoc)+1)),
     xlab="Age",
     ylab="Average absolute wealth",
     type="b"
)

# Fit real data ----

##Prepare data ----

#Age at first birth
#replace NAs with -99
for(j in 1:ncol(afr_matrix)){
  for(i in 1:nrow(afr_matrix)){
    if(is.na(afr_matrix[i,j])){
      afr_matrix[i,j] <- -99
    } else{
      afr_matrix[i,j] <- afr_matrix[i,j]
    }
  }
}
#check the data
afr_matrix

#Wealth
#matrix identifying missing wealth data
wealth_miss <- which(is.na(std_absw_matrix),arr.ind = T)
#check data
wealth_miss

#replace NAs with -99
for(j in 1:ncol(std_absw_matrix)){
  for(i in 1:nrow(std_absw_matrix)){
    if(is.na(std_absw_matrix[i,j])){
      std_absw_matrix[i,j] <- -99
    } else{
      std_absw_matrix[i,j] <- std_absw_matrix[i,j]
    }
  }
}
#check the data
std_absw_matrix

#Subset the data for realistic ages
#Subset wealth and AFB for those between zero years old and 50 years old.
#wealth
std_absw_restricted <- std_absw_matrix[,1:51] #Adding 1, since first column in the matrix is year 0
#AFB
afrs_restricted <- afr_matrix[,1:51] #Adding 1, since first column in the matrix is year 0
afrs_restricted[,1:10] <- -99 #turning the first 10 years to NAs because we do not need to model such ages for age at first birth
afrs_restricted
#missing wealth data
wealth_miss_restricted <- wealth_miss[wealth_miss[,2] <= 51,] #Adding 1, since first column in the matrix is year 0

#put all the data together
#create dataset
real_list <- list(N = nrow(afrs_restricted), #population size
                  A = ncol(afrs_restricted), #age
                  wealth = std_absw_restricted, #current absolute wealth
                  baby = afrs_restricted, #AFR
                  N_miss = nrow(wealth_miss_restricted), # number of missing values that need imputation
                  wealth_miss=wealth_miss_restricted) # matrix indicating missing wealth data
#check data
real_list

## Compile and fit model ----

# compile model

model_int2 <- cmdstan_model("Interaction/firstbaby_interaction_abs_diff.stan")

#fit model
fit_int2 <- model_int2$sample(data = real_list, 
                              chains = 4, 
                              parallel_chains = 15, 
                              adapt_delta = 0.95,
                              max_treedepth = 13,
                              init = 0)


# save fit 
fit_int2_csv <- rstan::read_stan_csv(fit_int2$output_files())
saveRDS(fit_int2_csv, "fit_int2_output.rds")
#load RDS file
rds_int2 <- readRDS("fit_int2_output.rds")
#extract samples
post_int2 <- extract.samples(rds_int2)

## Model diagnostics ----

#check trace of all parameters
#alpha
rstan::traceplot(rds_int2,pars="alpha")
#mu
traceplot(rds_int2,pars="mu") 
#mu_raw
traceplot(rds_int2,pars="mu_raw")
#mu_tau
rstan::traceplot(rds_int2,pars="mu_tau")
#mu_kappa
rstan::traceplot(rds_int2,pars="mu_kappa")
#mu_delta
rstan::traceplot(rds_int2,pars="mu_delta")
#beta_wealth_z
traceplot(rds_int2,pars="beta_wealth_z") 
#beta_wealth_sigma
traceplot(rds_int2,pars="beta_wealth_sigma") 
#gamma_wealth
traceplot(rds_int2,pars="gamma_wealth_z") 
#gamma_wealth
traceplot(rds_int2,pars="gamma_wealth_sigma") 
#zeta_wealth
traceplot(rds_int2,pars="zeta_wealth_z") 
#zeta_wealth
traceplot(rds_int2,pars="zeta_wealth_sigma") 

#summary of the model
#create summary tables of the different parameters

#alpha and hiper priors of Gaussian process
#create summary table for alpha and hiper priors of Gaussian process
tab_int2_alphagp <- precis(rds_int2,depth=2,pars=c("alpha",
                                                   "mu_raw",
                                                   "mu_tau",
                                                   "mu_delta"))
#check table
tab_int2_alphagp

#mu
#create summary table for mu
tab_int2_mu <- precis(rds_int2,depth=2,pars="mu")
#check table
tab_int2_mu
#plot it!
plot(tab_int2_mu)

#beta z
#create summary table for beta_z
tab_int2_beta_z <- precis(rds_int2,depth=2,pars="beta_wealth_z")
#check table
tab_int2_beta_z
#plot it!
plot(tab_int2_beta_z)

#beta sigma
#create summary table for beta_sigma
tab_int2_beta_sigma <- precis(rds_int2,depth=2,pars="beta_wealth_sigma")
#check table
tab_int2_beta_sigma

#gamma z
#create summary table for gamma_z
tab_int2_gamma_z <- precis(rds_int2,depth=2,pars="gamma_wealth_z")
#check table
tab_int2_gamma_z
#plot it!
plot(tab_int2_gamma_z)

#gamma sigma
#create summary table for gamma_sigma
tab_int2_gamma_sigma <- precis(rds_int2,depth=2,pars="gamma_wealth_sigma")
#check table
tab_int2_gamma_sigma

#zeta z
#create summary table for zeta_z
tab_int2_zeta_z <- precis(rds_int2,depth=2,pars="zeta_wealth_z")
#check table
tab_int2_zeta_z
#plot it!
plot(tab_int2_zeta_z)

#zeta sigma
#create summary table for zeta_sigma
tab_int2_zeta_sigma <- precis(rds_int2,depth=2,pars="zeta_wealth_sigma")
#check table
tab_int2_zeta_sigma

#Check correlation between wealth predictors

#beta versus delta
#create correlation matrix
cor1 <- round(cor(post_int2$beta_wealth_z,post_int2$gamma_wealth_z),3)
#plot it!
corrplot(cor1, "color", tl.col="black")

#beta versus epsion
#create correlation matrix
cor2 <- round(cor(post_int2$beta_wealth_z,post_int2$zeta_wealth_z),3)
#plot it!
corrplot(cor2, "color", tl.col="black")

#delta versus zeta
#create correlation matrix
cor3 <- round(cor(post_int2$gamma_wealth_z,post_int2$zeta_wealth_z),3)
#plot it!
corrplot(cor3, "color", tl.col="black")

# Plot the output of the model ----

## Current absolute Wealth ----

#simulate wealth values
simwealth_absw_int2 <- seq(from=round(min(post_int2$wealth_full),1),to=round(max(post_int2$wealth_full),1),length.out=nrow(std_absw_restricted)) #specify according to range and length related to sample size
simwealth_absw_int2
#get the deciles
deciles_absw <- as.numeric(quantile(simwealth_absw_int2,seq(0,1,0.5)))
deciles_absw

#colour palette
#numbers for color palette
palette <- palette.colors(9,"Okabe-Ito")
#select the numbers for color palette
palette_a<-palette[1:length(deciles_absw)]
palette_a

#shape of points
shape <- c(15:17)
#line type
type <- c(1:3)

#set parameters for a legend outside of the plot
par(mfrow=c(1,1),xpd=T,mar=c(5,5,4,12))

#plot empty plot
plot(c(0,1)~c(10,ncol(post_int2$mu)),
     ylab="Cumulative probability of first birth",
     xlab="Age",
     main="Current levels\nof material wealth",
     cex.axis=1.2,
     cex.lab=1.5,
     cex.main=1.5,
     type="n")
legend(53,1,c("Min.","Med.", "Max."),col=palette_a,lwd=3,pch=shape,lty=type,pt.cex = 1.5,cex=1.2,box.col = NA)

#add lines
for(k in 1:(length(deciles_absw))){
  #create matrix to store the data
  p_absw_int2 <- matrix(nrow=nrow(post_int2$mu),ncol=ncol(post_int2$mu))
  p_absw_int2
  #fill it in with values for age 25
  for(j in 1:ncol(post_int2$mu)){
    for(i in 1:nrow(post_int2$mu)){
      p_absw_int2[i,j] <- inv_logit(post_int2$alpha[i] + # global intercept
                                    post_int2$mu[i,j] + # age
                                    (post_int2$beta_wealth_z[i,j]*post_int2$beta_wealth_sigma[i])*deciles_absw[k] + # absolute wealth
                                    (post_int2$gamma_wealth_z[i,j]*post_int2$gamma_wealth_sigma[i])*0 + # absolute wealth change
                                    (post_int2$zeta_wealth_z[i,j]*post_int2$zeta_wealth_sigma[i])*(0*0) # interaction
      )
    }
  }
  #check data
  p_absw_int2
  #plot it!
  #prepare model prediction data
  plot_absw_int2 <- data.frame(age = 1:ncol(p_absw_int2),
                               median = apply(p_absw_int2, 2, median), 
                               upp = apply(p_absw_int2, 2, function(x) HPDI(x, prob = 0.9))[1, ], 
                               low = apply(p_absw_int2, 2, function(x) HPDI(x, prob = 0.9))[2, ]
  ) 
  #store data per decile
  assign(paste0("absw_",k),plot_absw_int2)
  
  # Calculate cumulative probabilities
  #create vectors
  cumulative_median_absw <- numeric(length(plot_absw_int2$median))
  cumulative_low_absw <- numeric(length(plot_absw_int2$low))
  cumulative_upp_absw <- numeric(length(plot_absw_int2$upp))
  #set the first probability
  cumulative_median_absw[1] <- plot_absw_int2$median[1]
  cumulative_low_absw[1] <- plot_absw_int2$low[1]
  cumulative_upp_absw[1] <- plot_absw_int2$upp[1]
  #calculate the cumulative probabilities for the other ages
  for (a in 2:length(plot_absw_int2$median)) {
    cumulative_median_absw[a] <- cumulative_median_absw[a-1] + (1 - cumulative_median_absw[a-1]) * plot_absw_int2$median[a]
    cumulative_low_absw[a] <- cumulative_low_absw[a-1] + (1 - cumulative_low_absw[a-1]) * plot_absw_int2$low[a]
    cumulative_upp_absw[a] <- cumulative_upp_absw[a-1] + (1 - cumulative_upp_absw[a-1]) * plot_absw_int2$upp[a]
  }
  #store data per decile
  assign(paste0("cumulative_median_absw_",k),cumulative_median_absw)
  assign(paste0("cumulative_low_absw_",k),cumulative_low_absw)
  assign(paste0("cumulative_upp_absw_",k),cumulative_upp_absw)
  
  #add median
  #add points
  points(cumulative_median_absw[11:51] ~ plot_absw_int2$age[11:51], col=palette_a[k], pch=shape[k], cex=1.5)
  #add lines
  lines(cumulative_median_absw[11:51] ~ plot_absw_int2$age[11:51], col=palette_a[k], lwd=3, lty=type[k])
  #add confidence intervals
  polygon(c(plot_absw_int2$age[11:51], rev(plot_absw_int2$age[11:51])), c(cumulative_low_absw[11:51], rev(cumulative_upp_absw[11:51])), col=alpha(palette_a[k], 0.25), border=NA)
}

## Short-term wealth variability ----

#simulate wealth values
simwealth_change_int2 <- seq(from=round(min(post_int2$wealth_change),1),to=round(max(post_int2$wealth_change),1),length.out=nrow(std_absw_restricted)) #specify according to range and length related to sample size
simwealth_change_int2
#get the deciles
deciles_diffw <- as.numeric(quantile(simwealth_change_int2,seq(0,1,0.5)))
deciles_diffw

#colour palette
#numbers for color palette
palette <- palette.colors(9,"Okabe-Ito")
#select the numbers for color palette
palette_b<-palette[4:(length(deciles_diffw)+3)]
palette_b

#shape of points
shape <- c(15:17)
#line type
type <- c(1:3)

#set parameters for a legend outside of the plot
par(mfrow=c(1,1),xpd=T,mar=c(5,5,4,12))

#plot empty plot
plot(c(0,1)~c(10,ncol(post_int2$mu)),
     ylab="Cumulative probability of first birth",
     xlab="Age",
     main="Short-term variability\nof material wealth",
     cex.axis=1.2,
     cex.lab=1.5,
     cex.main=1.5,
     type="n")
legend(53,1,c("Min.","Med.", "Max."),col=palette_b,lwd=3,pch=shape,lty=type,pt.cex = 1.5,cex=1.2,box.col = NA)

#add lines
for(k in 1:(length(deciles_diffw))){
  #create matrix to store the data
  p_diffw_int2 <- matrix(nrow=nrow(post_int2$mu),ncol=ncol(post_int2$mu))
  p_diffw_int2
  #fill it in with values for age 25
  for(j in 1:ncol(post_int2$mu)){
    for(i in 1:nrow(post_int2$mu)){
      p_diffw_int2[i,j] <- inv_logit(post_int2$alpha[i] + # global intercept
                                      post_int2$mu[i,j] + # age
                                      (post_int2$beta_wealth_z[i,j]*post_int2$beta_wealth_sigma[i])*0 + # absolute wealth
                                      (post_int2$gamma_wealth_z[i,j]*post_int2$gamma_wealth_sigma[i])*deciles_diffw[k] + # absolute wealth change
                                      (post_int2$zeta_wealth_z[i,j]*post_int2$zeta_wealth_sigma[i])*(0*0) # interaction
      )
    }
  }
  #check data
  p_diffw_int2
  #plot it!
  #prepare model prediction data
  plot_diffw_int2 <- data.frame(age = 1:ncol(p_diffw_int2),
                               median = apply(p_diffw_int2, 2, median), 
                               upp = apply(p_diffw_int2, 2, function(x) HPDI(x, prob = 0.9))[1, ], 
                               low = apply(p_diffw_int2, 2, function(x) HPDI(x, prob = 0.9))[2, ]
  ) 
  #store data per decile
  assign(paste0("diffw_",k),plot_diffw_int2)
  
  # Calculate cumulative probabilities
  #create vectors
  cumulative_median_diffw <- numeric(length(plot_diffw_int2$median))
  cumulative_low_diffw <- numeric(length(plot_diffw_int2$low))
  cumulative_upp_diffw <- numeric(length(plot_diffw_int2$upp))
  #set the first probability
  cumulative_median_diffw[1] <- plot_diffw_int2$median[1]
  cumulative_low_diffw[1] <- plot_diffw_int2$low[1]
  cumulative_upp_diffw[1] <- plot_diffw_int2$upp[1]
  #calculate the cumulative probabilities for the other ages
  for (a in 2:length(plot_diffw_int2$median)) {
    cumulative_median_diffw[a] <- cumulative_median_diffw[a-1] + (1 - cumulative_median_diffw[a-1]) * plot_diffw_int2$median[a]
    cumulative_low_diffw[a] <- cumulative_low_diffw[a-1] + (1 - cumulative_low_diffw[a-1]) * plot_diffw_int2$low[a]
    cumulative_upp_diffw[a] <- cumulative_upp_diffw[a-1] + (1 - cumulative_upp_diffw[a-1]) * plot_diffw_int2$upp[a]
  }
  #store data per decile
  assign(paste0("cumulative_median_diffw_",k),cumulative_median_diffw)
  assign(paste0("cumulative_low_diffw_",k),cumulative_low_diffw)
  assign(paste0("cumulative_upp_diffw_",k),cumulative_upp_diffw)
  
  #add median
  #add points
  points(cumulative_median_diffw[11:51] ~ plot_diffw_int2$age[11:51], col=palette_b[k], pch=shape[k], cex=1.5)
  #add lines
  lines(cumulative_median_diffw[11:51] ~ plot_diffw_int2$age[11:51], col=palette_b[k], lwd=3, lty=type[k])
  #add confidence intervals
  polygon(c(plot_diffw_int2$age[11:51], rev(plot_diffw_int2$age[11:51])), c(cumulative_low_diffw[11:51], rev(cumulative_upp_diffw[11:51])), col=alpha(palette_b[k], 0.25), border=NA)
}


## Interaction absolute and short-term ----

#simulate wealth values
simwealth_absw_int2 <- seq(from=round(min(post_int2$wealth_full),1),to=round(max(post_int2$wealth_full),1),length.out=nrow(std_absw_restricted)) #specify according to range and length related to sample size
simwealth_absw_int2
#get the deciles
deciles_absw_int2 <- as.numeric(quantile(simwealth_absw_int2,seq(0,1,0.5)))
deciles_absw_int2

#simulate wealth values
simwealth_diffw_int2 <- seq(from=round(min(post_int2$wealth_change),1),to=round(max(post_int2$wealth_change),1),length.out=nrow(std_absw_restricted)) #specify according to range and length related to sample size
simwealth_diffw_int2
#get the deciles
deciles_diffw_int2 <- as.numeric(quantile(simwealth_diffw_int2,seq(0,1,0.5)))
deciles_diffw_int2

### short-term wealth variability ----

layout(matrix(c(1,2,3,4,4,4), ncol=3,byrow=T),heights=c(4,0.175))

#colour palette
#numbers for color palette
palette <- palette.colors(9,"Okabe-Ito")
#select the numbers for color palette
palette_d<-palette[4:(length(deciles_diffw)+3)]
palette_d

#shape of points
shape <- c(15:17)
#line type
type <- c(1:3)

#### Min. absolute wealth ----

#set parameters for a legend outside of the plot
par(mar = c(5.1, 4.1, 4.1, 2.1))
#plot empty plot
plot(c(0,1)~c(10,ncol(post_int2$mu)),
     ylab="Cumulative probability of first birth",
     xlab="Age",
     main="short-term wealth variability with\nMinimum absolute wealth",
     cex.axis=1.2,
     cex.lab=1.5,
     cex.main=1.5,
     type="n")

#add lines
for(k in 1:(length(deciles_diffw_int2))){
  #create matrix to store the data
  p_int2_real <- matrix(nrow=nrow(post_int2$mu),ncol=ncol(post_int2$mu))
  p_int2_real
  #fill it in with values for age 25
  for(j in 1:ncol(post_int2$mu)){
    for(i in 1:nrow(post_int2$mu)){
      p_int2_real[i,j] <- inv_logit(post_int2$alpha[i] + #inv logit because originally is logit
                                      post_int2$mu[i,j] + #age
                                      (post_int2$beta_wealth_z[i,j]*post_int2$beta_wealth_sigma[i])*0 + #absolute wealth
                                      (post_int2$gamma_wealth_z[i,j]*post_int2$gamma_wealth_sigma[i])*0 + #moving variance
                                      (post_int2$zeta_wealth_z[i,j]*post_int2$zeta_wealth_sigma[i])*(deciles_absw_int2[1]*deciles_diffw_int2[k]) #interaction
      )  
    }
  }
  #check data
  p_int2_real
  #plot it!
  #prepare model prediction data
  plot_int2_real <- data.frame(age = 1:ncol(p_int2_real),
                               median = apply(p_int2_real, 2, median), 
                               upp = apply(p_int2_real, 2, function(x) HPDI(x, prob = 0.9))[1, ], 
                               low = apply(p_int2_real, 2, function(x) HPDI(x, prob = 0.9))[2, ]
  ) 
  # #store data per decile
  # assign(paste0("int2w_",i),plot_int2_real)
  
  # Calculate cumulative probabilities
  #create vectors
  cumulative_median_int2 <- numeric(length(plot_int2_real$median))
  cumulative_low_int2 <- numeric(length(plot_int2_real$low))
  cumulative_upp_int2 <- numeric(length(plot_int2_real$upp))
  #set the first probability
  cumulative_median_int2[1] <- plot_int2_real$median[1]
  cumulative_low_int2[1] <- plot_int2_real$low[1]
  cumulative_upp_int2[1] <- plot_int2_real$upp[1]
  #calculate the cumulative probabilities for the other ages
  for (a in 2:length(plot_int2_real$median)) {
    cumulative_median_int2[a] <- cumulative_median_int2[a-1] + (1 - cumulative_median_int2[a-1]) * plot_int2_real$median[a]
    cumulative_low_int2[a] <- cumulative_low_int2[a-1] + (1 - cumulative_low_int2[a-1]) * plot_int2_real$low[a]
    cumulative_upp_int2[a] <- cumulative_upp_int2[a-1] + (1 - cumulative_upp_int2[a-1]) * plot_int2_real$upp[a]
  }
  # #store data per decile
  # assign(paste0("cumulative_median_int2_",i),cumulative_median_int2)
  # assign(paste0("cumulative_low_int2_",i),cumulative_low_int2)
  # assign(paste0("cumulative_upp_int2_",i),cumulative_upp_int2)
  
  #add median
  #add points
  points(cumulative_median_int2[11:51] ~ plot_int2_real$age[11:51], col=palette_d[k], pch=shape[k], cex=1.5)
  #add lines
  lines(cumulative_median_int2[11:51] ~ plot_int2_real$age[11:51], col=palette_d[k], lwd=3, lty=type[k])
  #add confidence intervals
  polygon(c(plot_int2_real$age[11:51], rev(plot_int2_real$age[11:51])), c(cumulative_low_int2[11:51], rev(cumulative_upp_int2[11:51])), col=alpha(palette_d[k], 0.25), border=NA)
}

#### Mid. absolute wealth ----

#set parameters for a legend outside of the plot
par(mar = c(5.1, 4.1, 4.1, 2.1))
#plot empty plot
plot(c(0,1)~c(10,ncol(post_int2$mu)),
     ylab="Cumulative probability of first birth",
     xlab="Age",
     main="short-term wealth variability with\nMiddle absolute wealth",
     cex.axis=1.2,
     cex.lab=1.5,
     cex.main=1.5,
     type="n")

#add lines
for(k in 1:(length(deciles_diffw_int2))){
  #create matrix to store the data
  p_int2_real <- matrix(nrow=nrow(post_int2$mu),ncol=ncol(post_int2$mu))
  p_int2_real
  #fill it in with values for age 25
  for(j in 1:ncol(post_int2$mu)){
    for(i in 1:nrow(post_int2$mu)){
      p_int2_real[i,j] <- inv_logit(post_int2$alpha[i] + #inv logit because originally is logit
                                      post_int2$mu[i,j] + #age
                                      (post_int2$beta_wealth_z[i,j]*post_int2$beta_wealth_sigma[i])*0 + #absolute wealth
                                      (post_int2$gamma_wealth_z[i,j]*post_int2$gamma_wealth_sigma[i])*0 + #moving variance
                                      (post_int2$zeta_wealth_z[i,j]*post_int2$zeta_wealth_sigma[i])*(deciles_absw_int2[2]*deciles_diffw_int2[k]) #interaction
      )  
    }
  }
  #check data
  p_int2_real
  #plot it!
  #prepare model prediction data
  plot_int2_real <- data.frame(age = 1:ncol(p_int2_real),
                               median = apply(p_int2_real, 2, median), 
                               upp = apply(p_int2_real, 2, function(x) HPDI(x, prob = 0.9))[1, ], 
                               low = apply(p_int2_real, 2, function(x) HPDI(x, prob = 0.9))[2, ]
  ) 
  # #store data per decile
  # assign(paste0("int2w_",i),plot_int2_real)
  
  # Calculate cumulative probabilities
  #create vectors
  cumulative_median_int2 <- numeric(length(plot_int2_real$median))
  cumulative_low_int2 <- numeric(length(plot_int2_real$low))
  cumulative_upp_int2 <- numeric(length(plot_int2_real$upp))
  #set the first probability
  cumulative_median_int2[1] <- plot_int2_real$median[1]
  cumulative_low_int2[1] <- plot_int2_real$low[1]
  cumulative_upp_int2[1] <- plot_int2_real$upp[1]
  #calculate the cumulative probabilities for the other ages
  for (a in 2:length(plot_int2_real$median)) {
    cumulative_median_int2[a] <- cumulative_median_int2[a-1] + (1 - cumulative_median_int2[a-1]) * plot_int2_real$median[a]
    cumulative_low_int2[a] <- cumulative_low_int2[a-1] + (1 - cumulative_low_int2[a-1]) * plot_int2_real$low[a]
    cumulative_upp_int2[a] <- cumulative_upp_int2[a-1] + (1 - cumulative_upp_int2[a-1]) * plot_int2_real$upp[a]
  }
  # #store data per decile
  # assign(paste0("cumulative_median_int2_",i),cumulative_median_int2)
  # assign(paste0("cumulative_low_int2_",i),cumulative_low_int2)
  # assign(paste0("cumulative_upp_int2_",i),cumulative_upp_int2)
  
  #add median
  #add points
  points(cumulative_median_int2[11:51] ~ plot_int2_real$age[11:51], col=palette_d[k], pch=shape[k], cex=1.5)
  #add lines
  lines(cumulative_median_int2[11:51] ~ plot_int2_real$age[11:51], col=palette_d[k], lwd=3, lty=type[k])
  #add confidence intervals
  polygon(c(plot_int2_real$age[11:51], rev(plot_int2_real$age[11:51])), c(cumulative_low_int2[11:51], rev(cumulative_upp_int2[11:51])), col=alpha(palette_d[k], 0.25), border=NA)
}

#### Max. absolute wealth ----

#set parameters for a legend outside of the plot
par(mar = c(5.1, 4.1, 4.1, 2.1))
#plot empty plot
plot(c(0,1)~c(10,ncol(post_int2$mu)),
     ylab="Cumulative probability of first birth",
     xlab="Age",
     main="short-term wealth variability with\nMaximum absolute wealth",
     cex.axis=1.2,
     cex.lab=1.5,
     cex.main=1.5,
     type="n")

#add lines
for(k in 1:(length(deciles_diffw_int2))){
  #create matrix to store the data
  p_int2_real <- matrix(nrow=nrow(post_int2$mu),ncol=ncol(post_int2$mu))
  p_int2_real
  #fill it in with values for age 25
  for(j in 1:ncol(post_int2$mu)){
    for(i in 1:nrow(post_int2$mu)){
      p_int2_real[i,j] <- inv_logit(post_int2$alpha[i] + #inv logit because originally is logit
                                      post_int2$mu[i,j] + #age
                                      (post_int2$beta_wealth_z[i,j]*post_int2$beta_wealth_sigma[i])*0 + #absolute wealth
                                      (post_int2$gamma_wealth_z[i,j]*post_int2$gamma_wealth_sigma[i])*0 + #moving variance
                                      (post_int2$zeta_wealth_z[i,j]*post_int2$zeta_wealth_sigma[i])*(deciles_absw_int2[3]*deciles_diffw_int2[k]) #interaction
      )  
    }
  }
  #check data
  p_int2_real
  #plot it!
  #prepare model prediction data
  plot_int2_real <- data.frame(age = 1:ncol(p_int2_real),
                               median = apply(p_int2_real, 2, median), 
                               upp = apply(p_int2_real, 2, function(x) HPDI(x, prob = 0.9))[1, ], 
                               low = apply(p_int2_real, 2, function(x) HPDI(x, prob = 0.9))[2, ]
  ) 
  # #store data per decile
  # assign(paste0("int2w_",i),plot_int2_real)
  
  # Calculate cumulative probabilities
  #create vectors
  cumulative_median_int2 <- numeric(length(plot_int2_real$median))
  cumulative_low_int2 <- numeric(length(plot_int2_real$low))
  cumulative_upp_int2 <- numeric(length(plot_int2_real$upp))
  #set the first probability
  cumulative_median_int2[1] <- plot_int2_real$median[1]
  cumulative_low_int2[1] <- plot_int2_real$low[1]
  cumulative_upp_int2[1] <- plot_int2_real$upp[1]
  #calculate the cumulative probabilities for the other ages
  for (a in 2:length(plot_int2_real$median)) {
    cumulative_median_int2[a] <- cumulative_median_int2[a-1] + (1 - cumulative_median_int2[a-1]) * plot_int2_real$median[a]
    cumulative_low_int2[a] <- cumulative_low_int2[a-1] + (1 - cumulative_low_int2[a-1]) * plot_int2_real$low[a]
    cumulative_upp_int2[a] <- cumulative_upp_int2[a-1] + (1 - cumulative_upp_int2[a-1]) * plot_int2_real$upp[a]
  }
  # #store data per decile
  # assign(paste0("cumulative_median_int2_",i),cumulative_median_int2)
  # assign(paste0("cumulative_low_int2_",i),cumulative_low_int2)
  # assign(paste0("cumulative_upp_int2_",i),cumulative_upp_int2)
  
  #add median
  #add points
  points(cumulative_median_int2[11:51] ~ plot_int2_real$age[11:51], col=palette_d[k], pch=shape[k], cex=1.5)
  #add lines
  lines(cumulative_median_int2[11:51] ~ plot_int2_real$age[11:51], col=palette_d[k], lwd=3, lty=type[k])
  #add confidence intervals
  polygon(c(plot_int2_real$age[11:51], rev(plot_int2_real$age[11:51])), c(cumulative_low_int2[11:51], rev(cumulative_upp_int2[11:51])), col=alpha(palette_d[k], 0.25), border=NA)
}

#add legend
# Add the legend in the last row
par(mar = c(0, 0, 0, 0))  # Remove margins for the legend plot
plot.new()  # Create a new empty plot for the legend
legend("center",c("Null","Mid. var.", "Max. var."),col=palette_d,lwd=3,pch=shape,lty=type,pt.cex = 1.5,cex=1.2,horiz=T)

### Current absolute wealth ----

layout(matrix(c(1,2,3,4,4,4), ncol=3,byrow=T),heights=c(4,0.175))

#colour palette
#numbers for color palette
palette <- palette.colors(9,"Okabe-Ito")
#select the numbers for color palette
palette_e<-palette[1:length(deciles_absw_int2)]
palette_e

#shape of points
shape <- c(15:17)
#line type
type <- c(1:3)

#### Min. short-term wealth variability ----

#set parameters for a legend outside of the plot
par(mar = c(5.1, 4.1, 4.1, 2.1))
#plot empty plot
plot(c(0,1)~c(10,ncol(post_int2$mu)),
     ylab="Cumulative probability of first birth",
     xlab="Age",
     main="Current absolute wealth with\nMinimum short-term variability",
     cex.axis=1.2,
     cex.lab=1.5,
     cex.main=1.5,
     type="n")

#add lines
for(k in 1:(length(deciles_diffw_int2))){
  #create matrix to store the data
  p_int2_real <- matrix(nrow=nrow(post_int2$mu),ncol=ncol(post_int2$mu))
  p_int2_real
  #fill it in with values for age 25
  for(j in 1:ncol(post_int2$mu)){
    for(i in 1:nrow(post_int2$mu)){
      p_int2_real[i,j] <- inv_logit(post_int2$alpha[i] + #inv logit because originally is logit
                                      post_int2$mu[i,j] + #age
                                      (post_int2$beta_wealth_z[i,j]*post_int2$beta_wealth_sigma[i])*0 + #absolute wealth
                                      (post_int2$gamma_wealth_z[i,j]*post_int2$gamma_wealth_sigma[i])*0 + #moving variance
                                      (post_int2$zeta_wealth_z[i,j]*post_int2$zeta_wealth_sigma[i])*(deciles_absw_int2[k]*deciles_diffw_int2[1]) #interaction
      )  
    }
  }
  #check data
  p_int2_real
  #plot it!
  #prepare model prediction data
  plot_int2_real <- data.frame(age = 1:ncol(p_int2_real),
                               median = apply(p_int2_real, 2, median), 
                               upp = apply(p_int2_real, 2, function(x) HPDI(x, prob = 0.9))[1, ], 
                               low = apply(p_int2_real, 2, function(x) HPDI(x, prob = 0.9))[2, ]
  ) 
  # #store data per decile
  # assign(paste0("int2w_",i),plot_int2_real)
  
  # Calculate cumulative probabilities
  #create vectors
  cumulative_median_int2 <- numeric(length(plot_int2_real$median))
  cumulative_low_int2 <- numeric(length(plot_int2_real$low))
  cumulative_upp_int2 <- numeric(length(plot_int2_real$upp))
  #set the first probability
  cumulative_median_int2[1] <- plot_int2_real$median[1]
  cumulative_low_int2[1] <- plot_int2_real$low[1]
  cumulative_upp_int2[1] <- plot_int2_real$upp[1]
  #calculate the cumulative probabilities for the other ages
  for (a in 2:length(plot_int2_real$median)) {
    cumulative_median_int2[a] <- cumulative_median_int2[a-1] + (1 - cumulative_median_int2[a-1]) * plot_int2_real$median[a]
    cumulative_low_int2[a] <- cumulative_low_int2[a-1] + (1 - cumulative_low_int2[a-1]) * plot_int2_real$low[a]
    cumulative_upp_int2[a] <- cumulative_upp_int2[a-1] + (1 - cumulative_upp_int2[a-1]) * plot_int2_real$upp[a]
  }
  # #store data per decile
  # assign(paste0("cumulative_median_int2_",i),cumulative_median_int2)
  # assign(paste0("cumulative_low_int2_",i),cumulative_low_int2)
  # assign(paste0("cumulative_upp_int2_",i),cumulative_upp_int2)
  
  #add median
  #add points
  points(cumulative_median_int2[11:51] ~ plot_int2_real$age[11:51], col=palette_e[k], pch=shape[k], cex=1.5)
  #add lines
  lines(cumulative_median_int2[11:51] ~ plot_int2_real$age[11:51], col=palette_e[k], lwd=3, lty=type[k])
  #add confidence intervals
  polygon(c(plot_int2_real$age[11:51], rev(plot_int2_real$age[11:51])), c(cumulative_low_int2[11:51], rev(cumulative_upp_int2[11:51])), col=alpha(palette_e[k], 0.25), border=NA)
}

#### Mid. short-term wealth variability ----

#set parameters for a legend outside of the plot
par(mar = c(5.1, 4.1, 4.1, 2.1))
#plot empty plot
plot(c(0,1)~c(10,ncol(post_int2$mu)),
     ylab="Cumulative probability of first birth",
     xlab="Age",
     main="Current absolute wealth with\nMiddle short-term variability",
     cex.axis=1.2,
     cex.lab=1.5,
     cex.main=1.5,
     type="n")

#add lines
for(k in 1:(length(deciles_diffw_int2))){
  #create matrix to store the data
  p_int2_real <- matrix(nrow=nrow(post_int2$mu),ncol=ncol(post_int2$mu))
  p_int2_real
  #fill it in with values for age 25
  for(j in 1:ncol(post_int2$mu)){
    for(i in 1:nrow(post_int2$mu)){
      p_int2_real[i,j] <- inv_logit(post_int2$alpha[i] + #inv logit because originally is logit
                                      post_int2$mu[i,j] + #age
                                      (post_int2$beta_wealth_z[i,j]*post_int2$beta_wealth_sigma[i])*0 + #absolute wealth
                                      (post_int2$gamma_wealth_z[i,j]*post_int2$gamma_wealth_sigma[i])*0 + #moving variance
                                      (post_int2$zeta_wealth_z[i,j]*post_int2$zeta_wealth_sigma[i])*(deciles_absw_int2[k]*deciles_diffw_int2[2]) #interaction
      )  
    }
  }
  #check data
  p_int2_real
  #plot it!
  #prepare model prediction data
  plot_int2_real <- data.frame(age = 1:ncol(p_int2_real),
                               median = apply(p_int2_real, 2, median), 
                               upp = apply(p_int2_real, 2, function(x) HPDI(x, prob = 0.9))[1, ], 
                               low = apply(p_int2_real, 2, function(x) HPDI(x, prob = 0.9))[2, ]
  ) 
  # #store data per decile
  # assign(paste0("int2w_",i),plot_int2_real)
  
  # Calculate cumulative probabilities
  #create vectors
  cumulative_median_int2 <- numeric(length(plot_int2_real$median))
  cumulative_low_int2 <- numeric(length(plot_int2_real$low))
  cumulative_upp_int2 <- numeric(length(plot_int2_real$upp))
  #set the first probability
  cumulative_median_int2[1] <- plot_int2_real$median[1]
  cumulative_low_int2[1] <- plot_int2_real$low[1]
  cumulative_upp_int2[1] <- plot_int2_real$upp[1]
  #calculate the cumulative probabilities for the other ages
  for (a in 2:length(plot_int2_real$median)) {
    cumulative_median_int2[a] <- cumulative_median_int2[a-1] + (1 - cumulative_median_int2[a-1]) * plot_int2_real$median[a]
    cumulative_low_int2[a] <- cumulative_low_int2[a-1] + (1 - cumulative_low_int2[a-1]) * plot_int2_real$low[a]
    cumulative_upp_int2[a] <- cumulative_upp_int2[a-1] + (1 - cumulative_upp_int2[a-1]) * plot_int2_real$upp[a]
  }
  # #store data per decile
  # assign(paste0("cumulative_median_int2_",i),cumulative_median_int2)
  # assign(paste0("cumulative_low_int2_",i),cumulative_low_int2)
  # assign(paste0("cumulative_upp_int2_",i),cumulative_upp_int2)
  
  #add median
  #add points
  points(cumulative_median_int2[11:51] ~ plot_int2_real$age[11:51], col=palette_e[k], pch=shape[k], cex=1.5)
  #add lines
  lines(cumulative_median_int2[11:51] ~ plot_int2_real$age[11:51], col=palette_e[k], lwd=3, lty=type[k])
  #add confidence intervals
  polygon(c(plot_int2_real$age[11:51], rev(plot_int2_real$age[11:51])), c(cumulative_low_int2[11:51], rev(cumulative_upp_int2[11:51])), col=alpha(palette_e[k], 0.25), border=NA)
}

#### Max. short-term wealth variability ----

#set parameters for a legend outside of the plot
par(mar = c(5.1, 4.1, 4.1, 2.1))
#plot empty plot
plot(c(0,1)~c(10,ncol(post_int2$mu)),
     ylab="Cumulative probability of first birth",
     xlab="Age",
     main="Current absolute wealth with\nMaximum short-term variability",
     cex.axis=1.2,
     cex.lab=1.5,
     cex.main=1.5,
     type="n")

#add lines
for(k in 1:(length(deciles_diffw_int2))){
  #create matrix to store the data
  p_int2_real <- matrix(nrow=nrow(post_int2$mu),ncol=ncol(post_int2$mu))
  p_int2_real
  #fill it in with values for age 25
  for(j in 1:ncol(post_int2$mu)){
    for(i in 1:nrow(post_int2$mu)){
      p_int2_real[i,j] <- inv_logit(post_int2$alpha[i] + #inv logit because originally is logit
                                      post_int2$mu[i,j] + #age
                                      (post_int2$beta_wealth_z[i,j]*post_int2$beta_wealth_sigma[i])*0 + #absolute wealth
                                      (post_int2$gamma_wealth_z[i,j]*post_int2$gamma_wealth_sigma[i])*0 + #moving variance
                                      (post_int2$zeta_wealth_z[i,j]*post_int2$zeta_wealth_sigma[i])*(deciles_absw_int2[k]*deciles_diffw_int2[3]) #interaction
      )  
    }
  }
  #check data
  p_int2_real
  #plot it!
  #prepare model prediction data
  plot_int2_real <- data.frame(age = 1:ncol(p_int2_real),
                               median = apply(p_int2_real, 2, median), 
                               upp = apply(p_int2_real, 2, function(x) HPDI(x, prob = 0.9))[1, ], 
                               low = apply(p_int2_real, 2, function(x) HPDI(x, prob = 0.9))[2, ]
  ) 
  # #store data per decile
  # assign(paste0("int2w_",i),plot_int2_real)
  
  # Calculate cumulative probabilities
  #create vectors
  cumulative_median_int2 <- numeric(length(plot_int2_real$median))
  cumulative_low_int2 <- numeric(length(plot_int2_real$low))
  cumulative_upp_int2 <- numeric(length(plot_int2_real$upp))
  #set the first probability
  cumulative_median_int2[1] <- plot_int2_real$median[1]
  cumulative_low_int2[1] <- plot_int2_real$low[1]
  cumulative_upp_int2[1] <- plot_int2_real$upp[1]
  #calculate the cumulative probabilities for the other ages
  for (a in 2:length(plot_int2_real$median)) {
    cumulative_median_int2[a] <- cumulative_median_int2[a-1] + (1 - cumulative_median_int2[a-1]) * plot_int2_real$median[a]
    cumulative_low_int2[a] <- cumulative_low_int2[a-1] + (1 - cumulative_low_int2[a-1]) * plot_int2_real$low[a]
    cumulative_upp_int2[a] <- cumulative_upp_int2[a-1] + (1 - cumulative_upp_int2[a-1]) * plot_int2_real$upp[a]
  }
  # #store data per decile
  # assign(paste0("cumulative_median_int2_",i),cumulative_median_int2)
  # assign(paste0("cumulative_low_int2_",i),cumulative_low_int2)
  # assign(paste0("cumulative_upp_int2_",i),cumulative_upp_int2)
  
  #add median
  #add points
  points(cumulative_median_int2[11:51] ~ plot_int2_real$age[11:51], col=palette_e[k], pch=shape[k], cex=1.5)
  #add lines
  lines(cumulative_median_int2[11:51] ~ plot_int2_real$age[11:51], col=palette_e[k], lwd=3, lty=type[k])
  #add confidence intervals
  polygon(c(plot_int2_real$age[11:51], rev(plot_int2_real$age[11:51])), c(cumulative_low_int2[11:51], rev(cumulative_upp_int2[11:51])), col=alpha(palette_e[k], 0.25), border=NA)
}

#add legend
# Add the legend in the last row
par(mar = c(0, 0, 0, 0))  # Remove margins for the legend plot
plot.new()  # Create a new empty plot for the legend
legend("center",c("Poor","Middle", "Rich"),col=palette_e,lwd=3,pch=shape,lty=type,pt.cex = 1.5,cex=1.2,horiz=T)

# Relative importance ----

#prepare data
relative_int2 <- precis(rds_int2,
                        depth=2,
                        pars=c("beta_wealth_sigma","gamma_wealth_sigma","zeta_wealth_sigma"))

#set parameters for a legend outside of the plot
par(mfrow=c(1,1),xpd=T,mar=c(5,5,5,5))
#plot it!
plot(c(0,1),c(0,3),
     main="Relative importance\nof wealth predictors",
     type="n",
     xlab="Value",
     ylab="Wealth predictor",
     yaxt="n",
     cex.axis=1.2,
     cex.lab=1.5,
     cex.main=1.5
) 
axis(2,at=seq(2.5,0.5,by=-1),
     labels=c(expression(sigma[beta]),
              expression(sigma[gamma]),
              expression(sigma[zeta])
     ),
     las=1,
     tick=F,
     cex.axis=1.2
)
segments(0,-0.1,0,3.1,lty="dashed",col="lightgrey")
segments(-0.01,0.5,1,0.5,lty="dashed",col="lightgrey")
segments(-0.01,1.5,1,1.5,lty="dashed",col="lightgrey")
segments(-0.01,2.5,1,2.5,lty="dashed",col="lightgrey")
points(relative_int2[,1],seq(2.5,0.5,by=-1),cex=2,pch=16,col=hcl.colors(3,"berlin"))
segments(relative_int2[1,1]-relative_int2[1,2],2.5,relative_int2[1,1]+relative_int2[1,2],2.5,lwd=3,col=hcl.colors(3,"berlin")[1])
segments(relative_int2[2,1]-relative_int2[2,2],1.5,relative_int2[2,1]+relative_int2[2,2],1.5,lwd=3,col=hcl.colors(3,"berlin")[2])
segments(relative_int2[3,1]-relative_int2[3,2],0.5,relative_int2[3,1]+relative_int2[3,2],0.5,lwd=3,col=hcl.colors(3,"berlin")[3])
