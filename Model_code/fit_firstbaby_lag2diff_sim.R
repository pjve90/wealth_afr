# Model with lagged effect of absolute wealth change (2 year) ----
...
theta_wealth<-c(rep(0,12),seq(from=0.1,to=-0.09,length=19),seq(from=-0.09,to=-0.1,length=11),rep(0,32))
theta_wealth
plot(theta_wealth~c(1:length(theta_wealth)))
...
std_theta_wealth
plot(std_theta_wealth~c(1:length(std_theta_wealth)))
    ...
    if(!is.na(afrs[i,j-1])){
      if(afrs[i,j-1] == 0){
        afr_prob <- mu_age[j]+ #age
          std_theta_wealth[j]*std_diffwealth[i,j-3] #lagged absolute wealth change by 2 years
...
points(cumprod(1-(mu_age+std_theta_wealth)),col=hcl.colors(length(palette),"temps")[palette[1]],pch=15) #mu+std_beta
lines(cumprod(1-(mu_age+std_theta_wealth)),col=hcl.colors(length(palette),"temps")[palette[1]],lwd=2) #mu+std_beta
...
#create data
data9 <- list(N = nrow(afrs), #population size
              A = ncol(afrs), #age
...
m9_add <- cmdstan_model("Model_code/firstbaby_lag2diff.stan")

# fit model

fit9_add <- m9_add$sample(data = data9, 
                          chains = 4, 
                          parallel_chains = 15, 
                          adapt_delta = 0.95,
                          max_treedepth = 13,
...
#alpha
rstan::traceplot(rds9_add,pars="alpha")
#mu
#traceplot(rds9_add,pars="mu") #only run if needed, because they are 91 plots
#mu_raw
#traceplot(rds9_add,pars="mu_raw") #only run if needed, because they are 91 plots
#mu_tau
rstan::traceplot(rds9_add,pars="mu_tau")
#mu_kappa
rstan::traceplot(rds9_add,pars="mu_kappa")
#mu_delta
rstan::traceplot(rds9_add,pars="mu_delta")
#btheta_wealth
#traceplot(rds9_add,pars="btheta_wealth") #only run if needed, because they are 91 plots

#summary of the model
#create summary table
#create summary table for alpha and hiper priors of Gaussian process
tabs9_add <- precis(rds9_add,depth=3,pars=c("alpha",
                                            "mu_raw",
                                            "mu_tau",
                                            "mu_delta"))
#check table
...
tabs9_add_mu <- precis(rds9_add,depth=3,pars="mu")
#check table
tabs9_add_mu
plot(tabs9_add_mu)
plot(cumprod(1-inv_logit(tabs9_add_mu[,1])),ylim=c(0,1))
#create summary table for beta
tabs9_add_theta <- precis(rds9_add,depth=3,pars="theta_wealth")
#check table
tabs9_add_theta
plot(tabs9_add_theta)
plot(cumprod(1-inv_logit(tabs9_add_theta[,1])),ylim=c(0,1))
...
#plot empty plot
plot(c(0,1)~c(0,ncol(post9_add$mu)),
     ylab="Cumulative probability of first birth",
     xlab="Age",
...
for(k in 1:(length(deciles))){
  ...
  p9_add_b <- matrix(nrow=nrow(post9_add$mu),ncol=ncol(post9_add$mu))
  p9_add_b
  #fill it in with values for age 25
  for(j in 1:ncol(post9_add$mu)){
    for(i in 1:nrow(post9_add$mu)){
      p9_add_b[i,j] <- inv_logit(post9_add$alpha[i] + #inv logit because originally is logit
                                   post9_add$mu[i,j] + #age
                                   post9_add$theta_wealth[i,j]*deciles[k]) #wealth
    }
  }
  #check data
  p9_add_b
  ...
  plot_data9_add_b <- data.frame(age = 1:ncol(p9_add_b),
                                 mean = apply(p9_add_b, 2, mean), 
                                 upp = apply(p9_add_b, 2, function(x) HPDI(x, prob = 0.9))[1, ], 
                                 low = apply(p9_add_b, 2, function(x) HPDI(x, prob = 0.9))[2, ]
  ) 
  #prepare afr probabilities from real data
  #create a matrix
  plot_afr9 <- afrs
  #change -99 to NAs
  for(j in 1:ncol(plot_afr9)){
    for(i in 1:nrow(plot_afr9)){
      if(plot_afr9[i,j]==-99){
        plot_afr9[i,j] <- NA
      }
    }
  ...
  points(cumprod(1-plot_data9_add_b$mean)~plot_data9_add_b$age,col=hcl.colors(length(palette),"temps")[palette_b[k]],pch=15)
  lines(cumprod(1-plot_data9_add_b$mean)~plot_data9_add_b$age,col=hcl.colors(length(palette),"temps")[palette_b[k]])
  polygon(c(plot_data9_add_b$age,rev(plot_data9_add_b$age)),c(cumprod(1-plot_data9_add_b$low),rev(cumprod(1-plot_data9_add_b$upp))),col=alpha(hcl.colors(length(palette),"temps")[palette_b[k]],0.5),border=NA)
}

## De-couple plot by min, mean, max ----

#simulate wealth values
simwealth_add <- seq(from=round(min(post9_add$wealth_full),1),to=round(max(post9_add$wealth_full),1),length.out=nrow(std_abswealth)) #specify according to range and length related to sample size
simwealth_add
...
#create matrix to store the data
p9_add_0_b <- matrix(nrow=nrow(post9_add$mu),ncol=ncol(post9_add$mu))
p9_add_0_b
#fill it in with values for age 25
for(j in 1:ncol(post9_add$mu)){
  for(i in 1:nrow(post9_add$mu)){
    p9_add_0_b[i,j] <- inv_logit(post9_add$alpha[i] + #inv logit because originally is logit
                                   post9_add$mu[i,j] + #age
                                   post9_add$theta_wealth[i,j]*deciles[1]) #wealth
  }
}
#check data
p9_add_0_b
#plot it!
#prepare model prediction data
plot_data9_add_0_b <- data.frame(age = 1:ncol(p9_add_0_b),
                                 mean = apply(p9_add_0_b, 2, mean), 
                                 upp = apply(p9_add_0_b, 2, function(x) HPDI(x, prob = 0.9))[1, ], 
                                 low = apply(p9_add_0_b, 2, function(x) HPDI(x, prob = 0.9))[2, ]
) 
#prepare afr probabilities from real data
#create a matrix
plot_afr9 <- afrs
...
points(cumprod(1-plot_data9_add_0_b$mean)~plot_data9_add_0_b$age,col=alpha(hcl.colors(length(palette),"temps")[palette_b[1]],0.75),pch=15)
lines(cumprod(1-plot_data9_add_0_b$mean)~plot_data9_add_0_b$age,col=alpha(hcl.colors(length(palette),"temps")[palette_b[1]],0.75),lwd=2)
polygon(c(plot_data9_add_0_b$age,rev(plot_data9_add_0_b$age)),c(cumprod(1-plot_data9_add_0_b$low),rev(cumprod(1-plot_data9_add_0_b$upp))),col=alpha(hcl.colors(length(palette),"temps")[palette_b[1]],0.5),border=NA)
...
#create matrix to store the data
p9_add_50_b <- matrix(nrow=nrow(post9_add$mu),ncol=ncol(post9_add$mu))
p9_add_50_b
#fill it in with values for age 25
for(j in 1:ncol(post9_add$mu)){
  for(i in 1:nrow(post9_add$mu)){
    p9_add_50_b[i,j] <- inv_logit(post9_add$alpha[i] + #inv logit because originally is logit
                                    post9_add$mu[i,j] + #age
                                    post9_add$theta_wealth[i,j]*deciles[2]) #wealth
  }
}
#check data
p9_add_50_b
#plot it!
#prepare model prediction data
plot_data9_add_50_b <- data.frame(age = 1:ncol(p9_add_50_b),
                                  mean = apply(p9_add_50_b, 2, mean), 
                                  upp = apply(p9_add_50_b, 2, function(x) HPDI(x, prob = 0.9))[1, ], 
                                  low = apply(p9_add_50_b, 2, function(x) HPDI(x, prob = 0.9))[2, ]
) 
#prepare afr probabilities from real data
#create a matrix
plot_afr9 <- afrs
...
points(cumprod(1-plot_data9_add_50_b$mean)~plot_data9_add_50_b$age,col=alpha(hcl.colors(length(palette),"temps")[palette_b[2]],0.75),pch=15)
lines(cumprod(1-plot_data9_add_50_b$mean)~plot_data9_add_50_b$age,col=alpha(hcl.colors(length(palette),"temps")[palette_b[2]],0.75),lwd=2)
polygon(c(plot_data9_add_50_b$age,rev(plot_data9_add_50_b$age)),c(cumprod(1-plot_data9_add_50_b$low),rev(cumprod(1-plot_data9_add_50_b$upp))),col=alpha(hcl.colors(length(palette),"temps")[palette_b[2]],0.5),border=NA)
...
#create matrix to store the data
p9_add_100_b <- matrix(nrow=nrow(post9_add$mu),ncol=ncol(post9_add$mu))
p9_add_100_b
#fill it in with values for age 25
for(j in 1:ncol(post9_add$mu)){
  for(i in 1:nrow(post9_add$mu)){
    p9_add_100_b[i,j] <- inv_logit(post9_add$alpha[i] + #inv logit because originally is logit
                                     post9_add$mu[i,j] + #age
                                     post9_add$theta_wealth[i,j]*deciles[3]) #wealth
  }
}
#check data
p9_add_100_b
#plot it!
#prepare model prediction data
plot_data9_add_100_b <- data.frame(age = 1:ncol(p9_add_100_b),
                                   mean = apply(p9_add_100_b, 2, mean), 
                                   upp = apply(p9_add_100_b, 2, function(x) HPDI(x, prob = 0.9))[1, ], 
                                   low = apply(p9_add_100_b, 2, function(x) HPDI(x, prob = 0.9))[2, ]
) 
#prepare afr probabilities from real data
#create a matrix
plot_afr9 <- afrs
...
points(cumprod(1-plot_data9_add_100_b$mean)~plot_data9_add_100_b$age,col=alpha(hcl.colors(length(palette),"temps")[palette_b[3]],0.75),pch=15)
lines(cumprod(1-plot_data9_add_100_b$mean)~plot_data9_add_100_b$age,col=alpha(hcl.colors(length(palette),"temps")[palette_b[3]],0.75),lwd=2)
polygon(c(plot_data9_add_100_b$age,rev(plot_data9_add_100_b$age)),c(cumprod(1-plot_data9_add_100_b$low),rev(cumprod(1-plot_data9_add_100_b$upp))),col=alpha(hcl.colors(length(palette),"temps")[palette_b[3]],0.5),border=NA)
