...
#simulate wealth values
simwealth_add <- seq(from=round(min(post5_add$wealth_full),1),to=round(max(post5_add$wealth_full),1),length.out=nrow(std_wealth)) #specify according to range and length related to sample size
simwealth_add
