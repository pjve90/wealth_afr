...
//Lagged absolute wealth by one year
  vector[N*A] laggwealth; // lagged absolute wealth
  
  for(i in 1:N*A){
    if(i == 1){
      laggwealth[i] = wealth_full[i] - wealth_full[i];
    }else{
      laggwealth[i] = wealth_full[i-1];
    }
  }
  ...
  for (a in 2:A) {
    ...
    if(baby[n,a] != -99){
      ...
      baby[n, a] ~ bernoulli_logit( // Prob of having your first child
        ...
        mu[a] + // age
        delta_wealth[a]*laggwealth[(n-1)*a+a] // lagged absolute wealth
        );
