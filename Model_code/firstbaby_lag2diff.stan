...
//Lagged effect of absolute wealth change (1 year)
  vector[N*A] lagg2diff; //lagged effect of absolute change
  
  for(i in 1:N*A){
    if(i == 1 || i== 2 || i == 3){
      lagg2diff[i] = wealth_full[i] - wealth_full[i];
    }else{
      lagg2diff[i] = wealth_full[i] - wealth_full[i-3];
    }
  }
  
  vector[N*A] std_lagg2diff =  ((log(abs(lagg2diff)+1)) - mean((log(abs(lagg2diff)+1)))) / sd((log(abs(lagg2diff)+1))) ; // absolute values, log-transform, and standardised
  ...
  for (a in 2:A) {
    ...
    if(baby[n,a] != -99){
      ...
      baby[n, a] ~ bernoulli_logit( // Prob of having your first child
        ...
        mu[a] + // age
        theta_wealth[a]*std_lagg2diff[(n-1)*a+a] // lagged absolute wealth
        );
