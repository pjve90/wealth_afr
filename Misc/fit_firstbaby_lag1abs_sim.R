# Model with lagged effect of absolute wealth (1 year) ----
    ...
    if(!is.na(afrs[i,j-1])){
      if(afrs[i,j-1] == 0){
        afr_prob <- mu_age[j]+ #age
          std_delta_wealth[j]*lagg1wealth[i,j] #lagged absolute wealth by 1 year
...
saveRDS(fit5_add, "firstbaby5_add.rds")
#load RDS file
rds5_add <- readRDS("firstbaby5_add.rds")
#extract samples
...
simwealth_add <- seq(from=round(min(lagg1wealth[which(lagg1wealth > -99)]),1),to=round(max(lagg1wealth[which(lagg1wealth > -99)]),1),length.out=nrow(lagg1wealth)) #specify according to range and length of wealth data
simwealth_add
#get the deciles
...
#simulate wealth values
simwealth_add <- seq(from=round(min(post5_add$wealth_full),1),to=round(max(post5_add$wealth_full),1),length.out=nrow(lagg1wealth)) #specify according to range and length related to sample size
simwealth_add
