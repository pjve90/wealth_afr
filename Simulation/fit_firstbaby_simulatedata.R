...
# for(individual in 1:nrow(simwealth)){
#   for(ages in 2:ncol(simwealth)){
#     alpha_miss<-rbeta(1,1,1)
#     beta_miss<-rnorm(1,mean=0,sd=1)
#     mu_miss<-alpha_miss*simwealth[individual,(ages-1)]+(1-alpha_miss)*beta_miss
#     sigma_miss<-abs(rnorm(1,mean=0,sd=1))
#     wealth_impute_z<-rnorm(1,mean=0,sd=1)
#     wealth_impute<-mu_miss+sigma_miss* wealth_impute_z
#     simwealth[individual,ages]<-wealth_impute
#   }
# }
#check the data
simwealth

#standardise the simulated wealth
std_simwealth <- matrix(standardize(simwealth),ncol=ncol(simwealth),nrow=nrow(simwealth))
...
plot(density(std_simwealth),ylim=c(0,0.45)) # simulated data is the black line
...
plot(std_simwealth[,14]~std_simwealth[,13])
...
for(individual in 1:nrow(simshorttermwealth)){
  for(ages in 3:ncol(std_simwealth)){
    simshorttermwealth[individual,ages]<-abs(std_simwealth[individual,ages]-std_simwealth[individual,ages-2])
  }
}
#check data
simshorttermwealth
...
for(individual in 1:nrow(simlongtermwealth)){
  for(ages in 11:ncol(std_simwealth)){
    simlongtermwealth[individual,ages]<-sd(as.numeric(std_simwealth[individual,(ages-10):(ages)]))
  }
}
#check data
simlongtermwealth
...
# We use the median wealth of each individual as the starting point for the simulations, to reflect the scale of the interindividual differences.
#Get the median of each individual
medianwealthperindividual_sim<-NA
for(individual in 1:nrow(std_simwealth)){
  medianwealthperindividual_sim[individual]<-median(std_simwealth[individual,],na.rm=T)
}
#check the data
medianwealthperindividual_sim
#If there are individuals without wealth data, sample random values from a normal(0,1) distribution since the data is standardised
medianwealthperindividual_sim[is.na(medianwealthperindividual_sim)]<-rnorm(sum(is.na(medianwealthperindividual_sim)),0,1)
#check the data
medianwealthperindividual_sim
...
for(individual in 1:nrow(aw_simbirth)){
  ...
  for(ages in 11:ncol(aw_simbirth)){
    ageprob<-logit(afr_age_baseline[ages])+std_simwealth[individual,ages]*aw_beta[ages]
...
std_simwealth_res<-std_simwealth[,1:40]
#standardise median wealth
std_median_wealth <- standardize(medianwealthperindividual_sim)

#prepare missing values
#location
wealth_miss <- which(std_simwealth_res == -99, arr.ind = TRUE)
#check data
dim(wealth_miss)
...
aw_full_simulated_list <- list(N = nrow(aw_simbirth_res), #population size
                               ...
                               wealth = as.matrix(std_simwealth_res), #current absolute wealth
                               baby = as.matrix(aw_simbirth_res), #AFR
                               median_wealth = std_median_wealth, # median wealth of each individual
...
par(mfrow=c(1,1),xpd=T,mar=c(5,5,4,9))
#current wealth
plot(c(30:1)~c(aw_full_tab_sim_beta_z[11:40,1]*aw_full_tab_sim_beta_sigma[1,1]),xlim=c(-2,2),main="Current wealth",yaxt="n",xlab=expression(beta~coefficients),ylab="Ages",pch=16)
...
legend(2.3,30,  # or specify x/y coordinates
       legend = c("Estimated", "Simulated"),
       col = c("black",hcl.colors(3,"zissou 1")[1]),  
       lwd = 3,
       lty = c(1, NA),  # NA for headers
       pch = 16,    # Symbols for Estimated only
       pt.cex = 1.5,
       cex = 1.2,
       box.col = NA,
       title = expression(beta~coefficients))

###### Cumulative probabilities plot ----

#simulate wealth values
std_simwealth_aw_full <- seq(from=round(min(post_aw_full$wealth_full),1),to=round(max(post_aw_full$wealth_full),1),length.out=nrow(std_simwealth_res)) #specify according to range and length related to sample size
std_simwealth_aw_full
#get the deciles
deciles_aw_full <- c(mean(std_simwealth_aw_full)-sd(std_simwealth_aw_full),
                     mean(std_simwealth_aw_full),
                     mean(std_simwealth_aw_full)+sd(std_simwealth_aw_full))
deciles_aw_full
...
#set parameters for a legend outside of the plot par(mfrow=c(1,1),xpd=T,mar=c(5,5,4,15))
...
plot(c(0,1)~c(11,ncol(post_aw_full$mu)),
     ...
     type="n")
# Combined legend
legend(42,1,  # or specify x/y coordinates
       legend = c("Estimated", "Min.", "Med.", "Max.",  # First group
                  "",
                  "Simulated", "Min.", "Med.", "Max."), # Second group
       col = c(NA, palette_a,NA,NA,palette_a),  # NA for section headers
       lwd = 3,
       lty = c(NA, type[1:3],NA, NA, type[4:6]),  # NA for headers
       pch = c(NA, shape,NA, NA, rep(NA, 3)),    # Symbols for Estimated only
       pt.cex = 1.5,
       cex = 1.2,
       box.col = NA,
...
#change -99 to NA
std_simwealth_res[std_simwealth_res==-99] <- NA
std_simwealth_res
#create matrix
simshorttermwealth_res <- matrix(NA,nrow=nrow(std_simwealth_res),ncol=ncol(std_simwealth_res))
#calculate the short-term wealth variability
for(individual in 1:nrow(std_simwealth_res)){
  for(ages in 3:ncol(std_simwealth_res)){
    simshorttermwealth_res[individual,ages]<-abs(std_simwealth_res[individual,ages]-std_simwealth_res[individual,ages-2])
  }
}
#check data
simshorttermwealth_res
#calculate mean and sd
#mean
wc_mean_obs <- mean(simshorttermwealth_res,na.rm=T)
wc_mean_obs
#sd
wc_sd_obs <- sd(simshorttermwealth_res,na.rm=T)
wc_sd_obs
#change NA to -99
std_simwealth_res[which(is.na(std_simwealth_res))] <- -99
std_simwealth_res
...
par(mfrow=c(1,1),xpd=T,mar=c(5,5,4,9))
#short-term wealth variability
plot(c(30:1)~c(sc_full_tab_sim_gamma_z[11:40,1]*sc_full_tab_sim_gamma_sigma[1,1]),xlim=c(-2,2),main="Short-term\nwealth variability",yaxt="n",xlab=expression(gamma~coefficients),ylab="Ages",pch=16)
axis(2,c(30:1),c(10:39))
for (i in 11:40){
  segments(
...
legend(2.3,30,  # or specify x/y coordinates
       legend = c("Estimated", "Simulated"),
       col = c("black",hcl.colors(3,"zissou 1")[2]),  
       lwd = 3,
       lty = c(1, NA),  # NA for headers
       pch = 16,    # Symbols for Estimated only
       pt.cex = 1.5,
       cex = 1.2,
       box.col = NA,
       title = expression(gamma~coefficients))

###### Cumulative probabilities plot ----

#simulate wealth values
std_simwealth_sc_full <- seq(from=round(min(post_sc_full$wealth_change_std),1),to=round(max(post_sc_full$wealth_change_std),1),length.out=nrow(std_simwealth_res)) #specify according to range and length related to sample size
std_simwealth_sc_full
#get the deciles
#get the deciles
deciles_sc_full <- c(mean(std_simwealth_sc_full)-sd(std_simwealth_sc_full),
                     mean(std_simwealth_sc_full),
                     mean(std_simwealth_sc_full)+sd(std_simwealth_sc_full))
deciles_sc_full
...
legend(42,1,  # or specify x/y coordinates
       legend = c("Estimated", "Min.", "Med.", "Max.",  # First group
                  "",
                  "Simulated", "Min.", "Med.", "Max."), # Second group
       col = c(NA, palette_b,NA,NA,palette_b),  # NA for section headers
       lwd = 3,
       lty = c(NA, type[1:3],NA, NA, type[4:6]),  # NA for headers
       pch = c(NA, shape,NA, NA, rep(NA, 3)),    # Symbols for Estimated only
       pt.cex = 1.5,
...
std_simwealth_res
#create matrix
simlongtermwealth_full <- matrix(NA,nrow=nrow(std_simwealth_res),ncol=ncol(std_simwealth_res))
#calculate the long-term wealth variability
for(individual in 1:nrow(std_simwealth_res)){
  for(ages in 11:ncol(std_simwealth_res)){
    simlongtermwealth_full[individual,ages]<-sd(as.numeric(std_simwealth[individual,(ages-10):(ages)]),na.rm=T)
  }
}
#check data
simlongtermwealth_full
#calculate mean and sd
#mean
lv_mean_obs <- mean(simlongtermwealth_full,na.rm=T)
lv_mean_obs
#sd
lv_sd_obs <- sd(simlongtermwealth_full,na.rm=T)
lv_sd_obs
#change NA to -99
std_simwealth_res[which(is.na(std_simwealth_res))] <- -99
std_simwealth_res
...
par(mfrow=c(1,1),xpd=T,mar=c(5,5,4,9))
#long-term wealth variability
plot(c(30:1)~c(lv_full_tab_sim_delta_z[11:40,1]*lv_full_tab_sim_delta_sigma[1,1]),xlim=c(-2,2),main="Long-term\nwealth variability",yaxt="n",xlab=expression(delta~coefficients),ylab="Ages",pch=16)
axis(2,c(30:1),c(10:39))
for (i in 11:40){
  segments(
...
legend(2.3,30,  # or specify x/y coordinates
       legend = c("Estimated", "Simulated"),
       col = c("black",hcl.colors(3,"zissou 1")[3]),  
       lwd = 3,
       lty = c(1, NA),  # NA for headers
       pch = 16,    # Symbols for Estimated only
       pt.cex = 1.5,
       cex = 1.2,
       box.col = NA,
       title = expression(delta~coefficients))

######Cumulative probabilities plot ----

#simulate wealth values
std_simwealth_lv_full <- seq(from=round(min(post_lv_full$wealth_msd_std),1),to=round(max(post_lv_full$wealth_msd_std),1),length.out=nrow(std_simwealth_res)) #specify according to range and length related to sample size
std_simwealth_lv_full
#get the deciles
deciles_lv_full <- c(mean(std_simwealth_lv_full)-sd(std_simwealth_lv_full),
                     mean(std_simwealth_lv_full),
                     mean(std_simwealth_lv_full)+sd(std_simwealth_lv_full))
deciles_lv_full
...
plot(c(0,1)~c(10,ncol(post_lv_full$mu)),
     ...
     type="n")
# Combined legend
legend(42,1.1,  # or specify x/y coordinates
       legend = c("Estimated", "Min.", "Med.", "Max.",  # First group
                  "",
                  "Simulated", "Min.", "Med.", "Max."), # Second group
       col = c(NA, palette_c,NA,NA,palette_c),  # NA for section headers
       lwd = 3,
       lty = c(NA, type[1:3],NA, NA, type[4:6]),  # NA for headers
       pch = c(NA, shape,NA, NA, rep(NA, 3)),    # Symbols for Estimated only
       pt.cex = 1.5,
...
for(individual in 1:nrow(all_simbirth)){
  ...
  for(ages in 13:ncol(all_simbirth)){
    ageprob<-logit(afr_age_baseline[ages])+std_simwealth[individual,ages]*all_beta[ages]+simshorttermwealth[individual,ages]*all_gamma[ages]+simlongtermwealth[individual,ages]*all_delta[ages]
    ageprob<-inv_logit(ageprob)
...
#current wealth
plot(c(30:1)~c(all_full_tab_sim_beta_z[11:40,1]*all_full_tab_sim_beta_sigma[1,1]),
     xlim=c(-2,2),
     main="Current\nwealth",
     yaxt="n",
...
#short-term wealth variability
plot(c(30:1)~c(all_full_tab_sim_gamma_z[11:40,1]*all_full_tab_sim_gamma_sigma[1,1]),
     xlim=c(-2,2),
     main="Short-term\nwealth variability",
     yaxt="n",
...
#long-term wealth variability
plot(c(30:1)~c(all_full_tab_sim_delta_z[11:40,1]*all_full_tab_sim_delta_sigma[1,1]),
     xlim=c(-2,2),
     main="Long-term\nwealth variability",
     yaxt="n",
...
legend("bottom", 
       legend = c("Estimated", NA, "Simulated", NA, expression(beta), expression(gamma), expression(delta)),
       col = c("black", "black", NA, NA, hcl.colors(3, "zissou 1")),
       lwd = c(NA, 2, NA, NA, 2, 2, 2),
       lty = c(NA, 1, NA, NA, 1, 1, 1),
       pch = c(NA, 16, NA, NA, 16, 16, 16),
       ncol = 7,
       bty = "n",
       xjust = 0.5,
       text.width = c(0.07, 0.01, 0.02, 0.01, 0.01, 0.01, 0.01),
       pt.cex = 1.5,
       cex = 1.5
)
...
#simulate wealth values
std_simwealth_aw_all_full <- seq(from=round(min(post_all_full$wealth_full),1),to=round(max(post_all_full$wealth_full),1),length.out=nrow(std_simwealth_res)) #specify according to range and length related to sample size
std_simwealth_aw_all_full
#get the deciles
deciles_aw_all_full <- c(mean(std_simwealth_aw_all_full)-sd(std_simwealth_aw_all_full),
                         mean(std_simwealth_aw_all_full),
                         mean(std_simwealth_aw_all_full)+sd(std_simwealth_aw_all_full))
deciles_aw_all_full
...
#plot empty plot
plot(c(0,1)~c(11,ncol(post_all_full$mu)),
     ylab="Cumulative probability of first birth",
     xlab="Age",
     main="Current levels\nof material wealth",
     cex.axis=1.5,
...
axis(1,
     at = seq(11, ncol(post_all_full$mu), by = 5), 
     labels = seq(10, ncol(post_all_full$mu)-1, by = 5),
     cex.axis=1.5
)
# Combined legend
legend(42,1,  # or specify x/y coordinates
...
#add simulated cumulative probabilities
lines(cumsum(std_ageprobs_poor)[11:40]~c(11:40),col=palette_a[1],lty=2,lwd=3)
#medium wealth
lines(cumsum(std_afr_age_baseline)[11:40]~c(11:40),col=palette_a[2],lty=2,lwd=3)
#maximum wealth
lines(cumsum(std_ageprobs_rich)[11:40]~c(11:40),col=palette_a[3],lty=2,lwd=3)
...
for(k in 1:(length(deciles_aw_all_full))){
  ...
  #add median
  #add points
  points(cumulative_median_absw[11:40] ~ plot_aw_all_full$age[11:40], col=palette_a[k], pch=16, cex=1.5)
  #add lines
  lines(cumulative_median_absw[11:40] ~ plot_aw_all_full$age[11:40], col=palette_a[k], lwd=3, lty=1)
  #add confidence intervals
  polygon(c(plot_absw_real$age[11:40], rev(plot_absw_real$age[11:40])), c(cumulative_low_absw[11:40], rev(cumulative_upp_absw[11:40])), col=alpha(palette_a[k], 0.1), border=alpha(palette_a[k],0.25))
}

###### Short-term wealth variability ----

#simulate wealth values
std_simwealth_sc_all_full <- seq(from=round(min(post_all_full$wealth_change_std),1),to=round(max(post_all_full$wealth_change_std),1),length.out=nrow(std_simwealth_res)) #specify according to range and length related to sample size
std_simwealth_sc_all_full
#get the deciles
deciles_sc_all_full <- c(mean(std_simwealth_sc_all_full)-sd(std_simwealth_sc_all_full),
                         mean(std_simwealth_sc_all_full),
                         mean(std_simwealth_sc_all_full)+sd(std_simwealth_sc_all_full))
deciles_sc_all_full
...
#add simulated cumulative probabilities
lines(cumsum(std_ageprobs_minsc)[11:40]~c(11:40),col=palette_b[1],lty=2,lwd=3)
#medium wealth
lines(cumsum(std_afr_age_baseline)[11:40]~c(11:40),col=palette_b[2],lty=2,lwd=3)
#maximum wealth
lines(cumsum(std_ageprobs_maxsc)[11:40]~c(11:40),col=palette_b[3],lty=2,lwd=3)
...
for(k in 1:(length(deciles_sc_all_full))){
  ...
  # Calculate cumulative probabilities
  #create vectors
  cumulative_median_diffw <- numeric(length(plot_sc_all_full$median))
  cumulative_low_diffw <- numeric(length(plot_sc_all_full$low))
  cumulative_upp_diffw <- numeric(length(plot_sc_all_full$upp))
  #set the first probability
  cumulative_median_diffw[1] <- plot_sc_all_full$median[1]
  cumulative_low_diffw[1] <- plot_sc_all_full$low[1]
  cumulative_upp_diffw[1] <- plot_sc_all_full$upp[1]
  #calculate the cumulative probabilities for the other ages
  for (a in 2:length(plot_sc_all_full$median)) {
    cumulative_median_diffw[a] <- cumulative_median_diffw[a-1] + (1 - cumulative_median_diffw[a-1]) * plot_sc_all_full$median[a]
    cumulative_low_diffw[a] <- cumulative_low_diffw[a-1] + (1 - cumulative_low_diffw[a-1]) * plot_sc_all_full$low[a]
    cumulative_upp_diffw[a] <- cumulative_upp_diffw[a-1] + (1 - cumulative_upp_diffw[a-1]) * plot_sc_all_full$upp[a]
  }
  #store data per decile
  ...
  #add median
  #add points
  points(cumulative_median_diffw[11:40] ~ plot_sc_all_full$age[11:40], col=palette_b[k], pch=16, cex=1.5)
  #add lines
  lines(cumulative_median_diffw[11:40] ~ plot_sc_all_full$age[11:40], col=palette_b[k], lwd=3, lty=1)
  #add confidence intervals
  polygon(c(plot_sc_all_full$age[11:40], rev(plot_sc_all_full$age[11:40])), c(cumulative_low_diffw[11:40], rev(cumulative_upp_diffw[11:40])), col=alpha(palette_b[k], 0.1), border=alpha(palette_b[k],0.25))
}

###### Long-term variability of wealth ----

#simulate wealth values
std_simwealth_lv_all_full <- seq(from=round(min(post_all_full$wealth_msd_std),1),to=round(max(post_all_full$wealth_msd_std),1),length.out=nrow(std_simwealth_res)) #specify according to range and length related to sample size
std_simwealth_lv_all_full
#get the deciles
deciles_lv_all_full <- c(mean(std_simwealth_lv_all_full)-sd(std_simwealth_lv_all_full),
                         mean(std_simwealth_lv_all_full),
                         mean(std_simwealth_lv_all_full)+sd(std_simwealth_lv_all_full))
deciles_lv_all_full
...
#add simulated cumulative probabilities
lines(cumsum(std_ageprobs_minlv)[11:40]~c(11:40),col=palette_c[1],lty=2,lwd=3)
#medium wealth
lines(cumsum(std_afr_age_baseline)[11:40]~c(11:40),col=palette_c[2],lty=2,lwd=3)
#maximum wealth
lines(cumsum(std_ageprobs_maxlv)[11:40]~c(11:40),col=palette_c[3],lty=2,lwd=3)
...
for(k in 1:(length(deciles_lv_all_full))){
  ...
  # Calculate cumulative probabilities
  #create vectors
  cumulative_median_msdw <- numeric(length(plot_lv_all_full$median))
  cumulative_low_msdw <- numeric(length(plot_lv_all_full$low))
  cumulative_upp_msdw <- numeric(length(plot_lv_all_full$upp))
  #set the first probability
  cumulative_median_msdw[1] <- plot_lv_all_full$median[1]
  cumulative_low_msdw[1] <- plot_lv_all_full$low[1]
  cumulative_upp_msdw[1] <- plot_lv_all_full$upp[1]
  #calculate the cumulative probabilities for the other ages
  for (a in 2:length(plot_lv_all_full$median)) {
    cumulative_median_msdw[a] <- cumulative_median_msdw[a-1] + (1 - cumulative_median_msdw[a-1]) * plot_lv_all_full$median[a]
    cumulative_low_msdw[a] <- cumulative_low_msdw[a-1] + (1 - cumulative_low_msdw[a-1]) * plot_lv_all_full$low[a]
    cumulative_upp_msdw[a] <- cumulative_upp_msdw[a-1] + (1 - cumulative_upp_msdw[a-1]) * plot_lv_all_full$upp[a]
  }
  #store data per decile
  ...
  #add median
  #add points
  points(cumulative_median_msdw[11:40] ~ plot_lv_all_full$age[11:40], col=palette_c[k], pch=16, cex=1.5)
  #add lines
  lines(cumulative_median_msdw[11:40] ~ plot_lv_all_full$age[11:40], col=palette_c[k], lwd=3, lty=1)
  #add confidence intervals
  polygon(c(plot_lv_all_full$age[11:40], rev(plot_lv_all_full$age[11:40])), c(cumulative_low_msdw[11:40], rev(cumulative_upp_msdw[11:40])), col=alpha(palette_c[k], 0.1), border=alpha(palette_c[k],0.25))
}
...
sim_wealth_imputation<-matrix(NA,ncol=ncol(std_simwealth),nrow=nrow(std_simwealth))
...
for(j in 1:nrow(sim_wealth_imputation)){
  ...
  sim_wealth_imputation[j,age_first_census]<-std_simwealth[j,age_first_census]
  sim_wealth_imputation[j,age_first_census+2]<-std_simwealth[j,age_first_census+2]
  sim_wealth_imputation[j,age_first_census+4]<-std_simwealth[j,age_first_census+4]
  sim_wealth_imputation[j,age_first_census+6]<-std_simwealth[j,age_first_census+6]
  sim_wealth_imputation[j,age_first_census+8]<-std_simwealth[j,age_first_census+8]
  sim_wealth_imputation[j,age_first_census+10]<-std_simwealth[j,age_first_census+10]
  sim_wealth_imputation[j,age_first_census+12]<-std_simwealth[j,age_first_census+12]
}
...
##Individual median with incomplete wealth data ----

# We use the median wealth of each individual as the starting point for the simulations, to reflect the scale of the interindividual differences.
#Get the median of each individual
medianwealthperindividual_imp<-NA
for(individual in 1:nrow(sim_wealth_imputation)){
  medianwealthperindividual_imp[individual]<-median(sim_wealth_imputation[individual,],na.rm=T)
}
#check the data
medianwealthperindividual_imp
#If there are individuals without wealth data, sample random values from a normal(0,1) distribution since the data is standardised
medianwealthperindividual_imp[is.na(medianwealthperindividual_imp)]<-rnorm(sum(is.na(medianwealthperindividual_imp)),0,1)
#check the data
medianwealthperindividual_imp
...
#standardise median wealth
std_median_wealth <- standardize(medianwealthperindividual_imp)
...
# We put all of this together in the list of data for the analyses
aw_imputed_simulated_list <- list(N = nrow(sim_wealth_imputation_res), #population size
                                  A = ncol(sim_wealth_imputation_res), #age
...
#alpha
rstan::traceplot(aw_imputed_rds_simulated,pars="alpha_miss")
#alpha
rstan::traceplot(aw_imputed_rds_simulated,pars="beta_miss")
#alpha
rstan::traceplot(aw_imputed_rds_simulated,pars="sigma_miss")
#mu
...
mean(post_aw_imputed$wealth_full)
sd(post_aw_imputed$wealth_full)
range(post_aw_imputed$wealth_full)
mean(post_aw_imputed$wealth_impute)
sd(post_aw_imputed$wealth_impute)
range(post_aw_imputed$wealth_impute)
mean(post_aw_imputed$alpha_miss)
sd(post_aw_imputed$alpha_miss)
mean(post_aw_imputed$beta_miss)
sd(post_aw_imputed$beta_miss)
mean(post_aw_imputed$sigma_miss)
sd(post_aw_imputed$sigma_miss)

plot(density(sim_wealth_imputation_res[sim_wealth_imputation_res != -99]), 
     main = "Observed vs Imputed")
lines(density(post_aw_imputed$wealth_full),col="red")
lines(density(post_aw_imputed$wealth_impute),col="blue")
...
par(mfrow=c(1,1),xpd=T,mar=c(5,5,4,9))
#current wealth
plot(c(30:1)~c(aw_imputed_tab_sim_beta_z[11:40,1]*aw_imputed_tab_sim_beta_sigma[1,1]),xlim=c(-2,2),main="Current wealth",yaxt="n",xlab=expression(beta~coefficients),ylab="Ages",pch=16)
axis(2,c(30:1),c(10:39))
for (i in 11:40){
  segments(
...
#simulate wealth values
std_simwealth_aw_imputed <- seq(from=round(min(post_aw_imputed$wealth_full),1),to=round(max(post_aw_imputed$wealth_full),1),length.out=nrow(sim_wealth_imputation_res)) #specify according to range and length related to sample size
std_simwealth_aw_imputed
#get the deciles
deciles_aw_imputed <- c(mean(std_simwealth_aw_imputed)-sd(std_simwealth_aw_imputed),
                        mean(std_simwealth_aw_imputed),
                        mean(std_simwealth_aw_imputed)+sd(std_simwealth_aw_imputed))
deciles_aw_imputed
...
#plot empty plot
plot(c(0,1)~c(10,ncol(post_aw_imputed$mu)),
     ylab="Cumulative probability of first birth",
     xlab="Age",
     main="Current levels\nof material wealth",
     cex.axis=1.2,
...
for(k in 1:(length(deciles_aw_imputed))){
  ...
  #add confidence intervals
  polygon(c(plot_aw_imputed$age[10:40], rev(plot_aw_imputed$age[10:40])), c(cumulative_low_absw[10:40], rev(cumulative_upp_absw[10:40])), col=alpha(palette_a[k], 0.15), border=NA)
  #add median
  #add points
  points(cumulative_median_absw[10:40] ~ plot_aw_imputed$age[10:40], col=palette_a[k], pch=shape[k], cex=1.5)
  #add lines
  lines(cumulative_median_absw[10:40] ~ plot_aw_imputed$age[10:40], col=palette_a[k], lwd=3, lty=type[k])
}

#Scenario 6: Short-term wealth variability with incomplete data ---- 
...
mean(post_sc_imputed$wealth_full)
sd(post_sc_imputed$wealth_full)
range(post_sc_imputed$wealth_full)
mean(post_sc_imputed$wealth_impute)
sd(post_sc_imputed$wealth_impute)
range(post_sc_imputed$wealth_impute)
mean(post_sc_imputed$alpha_miss)
sd(post_sc_imputed$alpha_miss)
# mean(post_sc_imputed$beta_miss)
# sd(post_sc_imputed$beta_miss)
mean(post_sc_imputed$sigma_miss)
sd(post_sc_imputed$sigma_miss)
...
par(mfrow=c(1,1),xpd=T,mar=c(5,5,4,9))
#Short-term\nwealth variability
plot(c(30:1)~c(sc_imputed_tab_sim_gamma_z[11:40,1]*sc_imputed_tab_sim_gamma_sigma[1,1]),xlim=c(-2,2),main="Short-term\nwealth variability",yaxt="n",xlab=expression(gamma~coefficients),ylab="Ages",pch=16)
axis(2,c(30:1),c(10:39))
for (i in 11:40){
  segments(
...
#simulate wealth values
std_simwealth_sc_imputed <- seq(from=round(min(post_sc_imputed$wealth_change_std),1),to=round(max(post_sc_imputed$wealth_change_std),1),length.out=nrow(sim_wealth_imputation_res)) #specify according to range and length related to sample size
std_simwealth_sc_imputed
#get the deciles
#get the deciles
deciles_sc_imputed <- c(mean(std_simwealth_sc_imputed)-sd(std_simwealth_sc_imputed),
                        mean(std_simwealth_sc_imputed),
                        mean(std_simwealth_sc_imputed)+sd(std_simwealth_sc_imputed))
deciles_sc_imputed
...
mean(post_lv_imputed$wealth_full)
sd(post_lv_imputed$wealth_full)
range(post_lv_imputed$wealth_full)
mean(post_lv_imputed$wealth_impute)
sd(post_lv_imputed$wealth_impute)
range(post_lv_imputed$wealth_impute)
mean(post_lv_imputed$alpha_miss)
sd(post_lv_imputed$alpha_miss)
mean(post_lv_imputed$sigma_miss)
sd(post_lv_imputed$sigma_miss)
...
par(mfrow=c(1,1),xpd=T,mar=c(5,5,4,9))
#long-term wealth variability
plot(c(30:1)~c(lv_imputed_tab_sim_delta_z[11:40,1]*lv_imputed_tab_sim_delta_sigma[1,1]),xlim=c(-2,2),main="Long-term\nwealth variability",yaxt="n",xlab=expression(delta~coefficients),ylab="Ages",pch=16)
axis(2,c(30:1),c(10:39))
for (i in 11:40){
  segments(
...
#simulate wealth values
std_simwealth_lv_imputed <- seq(from=round(min(post_lv_imputed$wealth_msd_std),1),to=round(max(post_lv_imputed$wealth_msd_std),1),length.out=nrow(std_simwealth_res)) #specify according to range and length related to sample size
std_simwealth_lv_imputed
#get the deciles
deciles_lv_imputed <- c(mean(std_simwealth_lv_imputed)-sd(std_simwealth_lv_imputed),
                        mean(std_simwealth_lv_imputed),
                        mean(std_simwealth_lv_imputed)+sd(std_simwealth_lv_imputed))
deciles_lv_imputed
...
mean(post_all_imputed$wealth_full)
sd(post_all_imputed$wealth_full)
range(post_all_imputed$wealth_full)
mean(post_all_imputed$wealth_impute)
sd(post_all_imputed$wealth_impute)
range(post_all_imputed$wealth_impute)
mean(post_all_imputed$wealth_change_std)
sd(post_all_imputed$wealth_change_std)
mean(post_all_imputed$wealth_msd_std)
sd(post_all_imputed$wealth_msd_std)
mean(post_all_imputed$alpha_miss)
sd(post_all_imputed$alpha_miss)
mean(post_all_imputed$sigma_miss)
sd(post_all_imputed$sigma_miss)
...
par(mfrow = c(1, 3), mar = c(5, 4, 4, 2) + 0.5, oma = c(1, 0, 0, 0), xpd = NA)
#current wealth
plot(c(30:1)~c(all_imputed_tab_sim_beta_z[11:40,1]*all_imputed_tab_sim_beta_sigma[1,1]),
     xlim=c(-2,2),
     main="Current\nwealth",
     yaxt="n",
     xlab=expression(beta~coefficients),
...
axis(2,c(30:1),c(10:39),cex.axis=1.2)
clip(-2, 2, 0, 31)
abline(v=0,lty=2)
for (i in 11:40){
  segments(
...
points(c(30:1)~all_beta[11:40],col=hcl.colors(3,"zissou 1")[1],pch=16)
#short-term wealth variability
plot(c(30:1)~c(all_imputed_tab_sim_gamma_z[11:40,1]*all_imputed_tab_sim_gamma_sigma[1,1]),
     xlim=c(-2,2),
     main="Short-term\nwealth variability",
     yaxt="n",
     xlab=expression(gamma~coefficients),
...
points(c(30:1)~all_gamma[11:40],col=hcl.colors(3,"zissou 1")[2],pch=16)
#long-term wealth variability
plot(c(30:1)~c(all_imputed_tab_sim_delta_z[11:40,1]*all_imputed_tab_sim_delta_sigma[1,1]),
     xlim=c(-2,2),
     main="Long-term\nwealth variability",
     yaxt="n",
     xlab=expression(delta~coefficients),
...
deciles_aw_all_imputed <- c(mean(std_simwealth_aw_all_imputed)-sd(std_simwealth_aw_all_imputed),
                            mean(std_simwealth_aw_all_imputed),
                            mean(std_simwealth_aw_all_imputed)+sd(std_simwealth_aw_all_imputed))
...
#plot empty plot
plot(c(0,1)~c(11,ncol(post_all_imputed$mu)),
     ylab="Cumulative probability of first birth",
     xlab="Age",
     main="Current levels\nof material wealth",
     cex.axis=1.5,
...
#x-axis should follow biological ages
axis(1,
     at = seq(11, ncol(post_all_imputed$mu), by = 5), 
     labels = seq(10, ncol(post_all_imputed$mu)-1, by = 5),
     cex.axis=1.5
)
# Combined legend
legend(42,1,  # or specify x/y coordinates
...
for(k in 1:(length(deciles_aw_all_imputed))){
  ...
  #add median
  #add points
  points(cumulative_median_absw[11:40] ~ plot_aw_all_imputed$age[11:40], col=palette_a[k], pch=16, cex=1.5)
  #add lines
  lines(cumulative_median_absw[11:40] ~ plot_aw_all_imputed$age[11:40], col=palette_a[k], lwd=3, lty=1)
  #add confidence intervals
  polygon(c(plot_absw_real$age[11:40], rev(plot_absw_real$age[11:40])), c(cumulative_low_absw[11:40], rev(cumulative_upp_absw[11:40])), col=alpha(palette_a[k], 0.1), border=alpha(palette_a[k],0.25))
}
...
deciles_sc_all_imputed <- c(mean(std_simwealth_sc_all_imputed)-sd(std_simwealth_sc_all_imputed),
                            mean(std_simwealth_sc_all_imputed),
                            mean(std_simwealth_sc_all_imputed)+sd(std_simwealth_sc_all_imputed))
...
for(k in 1:(length(deciles_sc_all_imputed))){
  ...
  # Calculate cumulative probabilities
  #create vectors
  cumulative_median_diffw <- numeric(length(plot_sc_all_imputed$median))
  cumulative_low_diffw <- numeric(length(plot_sc_all_imputed$low))
  cumulative_upp_diffw <- numeric(length(plot_sc_all_imputed$upp))
  #set the first probability
  cumulative_median_diffw[1] <- plot_sc_all_imputed$median[1]
  cumulative_low_diffw[1] <- plot_sc_all_imputed$low[1]
  cumulative_upp_diffw[1] <- plot_sc_all_imputed$upp[1]
  #calculate the cumulative probabilities for the other ages
  for (a in 2:length(plot_sc_all_imputed$median)) {
    cumulative_median_diffw[a] <- cumulative_median_diffw[a-1] + (1 - cumulative_median_diffw[a-1]) * plot_sc_all_imputed$median[a]
    cumulative_low_diffw[a] <- cumulative_low_diffw[a-1] + (1 - cumulative_low_diffw[a-1]) * plot_sc_all_imputed$low[a]
    cumulative_upp_diffw[a] <- cumulative_upp_diffw[a-1] + (1 - cumulative_upp_diffw[a-1]) * plot_sc_all_imputed$upp[a]
  }
  #store data per decile
  assign(paste0("cumulative_median_diffw_",k),cumulative_median_diffw)
  assign(paste0("cumulative_low_diffw_",k),cumulative_low_diffw)
  ...
  #add median
  #add points
  points(cumulative_median_diffw[11:40] ~ plot_sc_all_imputed$age[11:40], col=palette_b[k], pch=16, cex=1.5)
  #add lines
  lines(cumulative_median_diffw[11:40] ~ plot_sc_all_imputed$age[11:40], col=palette_b[k], lwd=3, lty=1)
  #add confidence intervals
  polygon(c(plot_sc_all_imputed$age[11:40], rev(plot_sc_all_imputed$age[11:40])), c(cumulative_low_diffw[11:40], rev(cumulative_upp_diffw[11:40])), col=alpha(palette_b[k], 0.1), border=alpha(palette_b[k],0.25))
}
...
deciles_lv_all_imputed <- c(mean(std_simwealth_lv_all_imputed)-sd(std_simwealth_lv_all_imputed),
                            mean(std_simwealth_lv_all_imputed),
                            mean(std_simwealth_lv_all_imputed)+sd(std_simwealth_lv_all_imputed))
...
for(k in 1:(length(deciles_lv_all_imputed))){
  ...
  # Calculate cumulative probabilities
  #create vectors
  cumulative_median_msdw <- numeric(length(plot_lv_all_imputed$median))
  cumulative_low_msdw <- numeric(length(plot_lv_all_imputed$low))
  cumulative_upp_msdw <- numeric(length(plot_lv_all_imputed$upp))
  #set the first probability
  cumulative_median_msdw[1] <- plot_lv_all_imputed$median[1]
  cumulative_low_msdw[1] <- plot_lv_all_imputed$low[1]
  cumulative_upp_msdw[1] <- plot_lv_all_imputed$upp[1]
  #calculate the cumulative probabilities for the other ages
  for (a in 2:length(plot_lv_all_imputed$median)) {
    cumulative_median_msdw[a] <- cumulative_median_msdw[a-1] + (1 - cumulative_median_msdw[a-1]) * plot_lv_all_imputed$median[a]
    cumulative_low_msdw[a] <- cumulative_low_msdw[a-1] + (1 - cumulative_low_msdw[a-1]) * plot_lv_all_imputed$low[a]
    cumulative_upp_msdw[a] <- cumulative_upp_msdw[a-1] + (1 - cumulative_upp_msdw[a-1]) * plot_lv_all_imputed$upp[a]
  }
  #store data per decile
  assign(paste0("cumulative_median_msdw_",k),cumulative_median_msdw)
  assign(paste0("cumulative_low_msdw_",k),cumulative_low_msdw)
  ...
  #add median
  #add points
  points(cumulative_median_msdw[11:40] ~ plot_lv_all_imputed$age[11:40], col=palette_c[k], pch=16, cex=1.5)
  #add lines
  lines(cumulative_median_msdw[11:40] ~ plot_lv_all_imputed$age[11:40], col=palette_c[k], lwd=3, lty=1)
  #add confidence intervals
  polygon(c(plot_lv_all_imputed$age[11:40], rev(plot_lv_all_imputed$age[11:40])), c(cumulative_low_msdw[11:40], rev(cumulative_upp_msdw[11:40])), col=alpha(palette_c[k], 0.1), border=alpha(palette_c[k],0.25))
}
